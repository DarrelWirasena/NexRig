...
@section('content')
    ...
    <div x-data="{
    ...
    }" x-init="window.addEventListener('scroll', () => checkScroll())" class="w-full relative">

        {{-- Kontainer dengan tinggi tetap agar tidak lompat --}}
        <div class="h-[120px] md:h-[130px] w-full relative">

            {{-- Elemen Bar --}}
            <div :class="isSticky ? 'fixed top-[75px] left-4 right-4 md:left-10 md:right-10 rounded-2xl shadow-2xl' :
                'relative w-full'"
                ...
                <div class="max-w-[1440px] mx-auto flex flex-col gap-4">
                    ...
                    <div
                        class="flex items-center h-10 bg-white/5 border border-white/10 rounded-xl overflow-hidden relative">
                        <form x-ref="searchForm" action="{{ route('products.index') }}" method="GET"
                            class="flex-1 flex items-center h-full px-3 relative">
                            ...
                            <input x-ref="searchInput" x-model="searchQuery" type="text" name="search"
                                placeholder="Search model..."
                                class="w-full bg-transparent border-none p-0 text-white text-sm focus:ring-0 outline-none placeholder-gray-500 pr-8">

                            <button type="button" x-show="searchQuery.length > 0" x-cloak @click="clearSearch()"
                        ...
                        <button @click="openSort = !openSort"
                            class="px-3 flex items-center justify-center h-full text-gray-400 hover:text-primary transition-colors">
                        ...
                        @foreach ($chips as $chip)
                            @php
                                ...
                                $url = $isActive
                                    ...
                                    : route(
                                        'products.index',
                                        array_merge(request()->except('page'), ['search' => $chip]),
                                    );
                            @endphp
        ...
        <div x-show="openFilter" x-cloak class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
            ...
            {{-- Backdrop (Gelap) --}}
            <div x-show="openFilter" x-transition.opacity @click="openFilter = false"
                class="fixed inset-0 bg-black/80 backdrop-blur-sm"></div>

            {{-- Panel Modal (Slide Up di mobile, Fade/Scale di desktop) --}}
            <div x-show="openFilter" x-transition:enter="transition ease-out duration-300 transform"
                            ...
                            @foreach ($categories as $cat)
                                @php
                                    ...
                                    $url = $isActive
                                        ...
                                        : route(
                                            'products.index',
                                            array_merge(request()->except('page'), ['category' => $cat->slug]),
                                        );
                                @endphp
                                <a href="{{ $url }}"
                            ...
                            @foreach ($prices as $val => $label)
                                @php
                                    ...
                                    $url = $isActive
                                        ...
                                        : route(
                                            'products.index',
                                            array_merge(request()->except('page'), ['price' => $val]),
                                        );
                                @endphp
                                <a href="{{ $url }}"
                                    class="px-3 py-1.5 rounded border text-xs font-bold transition-colors
                ...
                <div class="p-4 border-t border-white/10 shrink-0">
                    <a href="{{ route('products.index') }}"
                        class="w-full block text-center py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-colors">
                        Clear All Filters
                    </a>
                </div>
            </div>

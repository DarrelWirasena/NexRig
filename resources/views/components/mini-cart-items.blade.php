@if(count($items) > 0)
<div class="space-y-4">
    {{-- Loop variabel $items (bukan $cart lagi) --}}
    @foreach($items as $item)
    <div id="cart-item-{{ $item->row_id }}" class="relative flex gap-4 p-3 rounded-xl bg-[#0a0a0a] border border-white/10 group hover:border-primary/50 transition-colors">

        {{-- Gambar Produk --}}
        <div class="w-20 h-20 bg-white/5 rounded-lg overflow-hidden shrink-0 border border-white/5">
            {{-- Ubah $details['image'] jadi $item->image --}}
        ...
        <div class="flex-1 pr-6">
            <h4 class="text-white font-bold text-sm line-clamp-2 leading-tight group-hover:text-primary transition-colors">
                {{-- Ubah $details['name'] jadi $item->name --}}
                {{ $item->name }}
            </h4>
            ...
            <div class="flex justify-between items-end mt-3">
                ...
                <div class="flex items-center gap-2 bg-[#050505] rounded-lg border border-white/10 p-1">
                    ...
                    {{-- Minus --}}
                    <button onclick="updateCartQuantity('{{ $item->row_id }}', -1)"
                        class="w-5 h-5 flex items-center justify-center rounded bg-white/5 hover:bg-primary hover:text-white text-gray-500 transition-colors {{ $item->quantity <= 1 ? 'invisible pointer-events-none' : '' }}">
        ...
        {{-- Hapus --}}
        <button onclick="removeCartItem('{{ $item->row_id }}')"
...
@else
{{-- EMPTY STATE (Sama seperti sebelumnya, tidak perlu diubah) --}}
<div class="absolute inset-0 flex flex-col items-center justify-center text-center py-12 px-8 opacity-75">
    ...
    <a href="{{ route('products.index') }}" class="px-8 py-3 bg-white/5 hover:bg-primary hover:text-white text-gray-300 font-bold rounded-lg transition-all border border-white/10 text-[10px] tracking-widest uppercase flex items-center gap-2">
        Choose your Rig

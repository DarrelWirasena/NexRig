<aside id="sidebar" class="sidebar-transition fixed inset-y-0 left-0 z-40 w-72 bg-[#0a0a0a] border-r border-white/10 flex flex-col transform -translate-x-full lg:translate-x-0 lg:static lg:h-full lg:z-20 shadow-2xl lg:shadow-none no-bounce">
    ...
    <nav class="flex-1 p-6 space-y-2 overflow-y-auto no-bounce custom-sidebar-scroll">
        ...
        <a href="{{ route('profile.app') }}"
           class="nav-item {{ request()->routeIs('profile.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-400 transition-all uppercase tracking-wide">
            <span class="material-symbols-outlined">person</span> Profile Settings
        </a>

        <a href="{{ route('address.index') }}"
           class="nav-item {{ request()->routeIs('address.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-400 transition-all uppercase tracking-wide">
            <span class="material-symbols-outlined">location_on</span> Address Book
        </a>

        <a href="{{ route('orders.index') }}"
           class="nav-item {{ request()->routeIs('orders.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-400 transition-all uppercase tracking-wide">
            <span class="material-symbols-outlined">receipt_long</span> My Orders
        </a>

        {{-- WISHLIST --}}
        <a href="{{ route('wishlist.index') }}"
           class="nav-item {{ request()->routeIs('wishlist.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-400 transition-all uppercase tracking-wide">
            <span class="material-symbols-outlined"
                  style="font-variation-settings: 'FILL' {{ request()->routeIs('wishlist.*') ? '1' : '0' }}">
                favorite
            </span>
            My Wishlist
        </a>
        ...
        <a href="{{ route('support.history') }}"
           class="nav-item {{ request()->routeIs('support.*') ? 'active' : '' }} flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-gray-400 transition-all uppercase tracking-wide">

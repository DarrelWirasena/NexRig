...
@section('content')
    ...
    <div class="fixed inset-0 z-0 bg-[#050505]">
        ...
        <div
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl h-[400px] bg-cyan-600/10 blur-[100px] rounded-full">
        </div>
    </div>
    ...
    <div class="relative h-screen w-full flex items-center justify-center p-4">
        ...
        <div class="relative z-20 w-full max-w-[360px] flex flex-col items-center">
            ...
            <div class="text-center mb-6 w-full relative z-50">
                <div
                    class="inline-flex items-center gap-2 px-3 py-1 rounded-sm border border-cyan-500/30 bg-cyan-500/10 mb-4">
            ...
            {{-- OTP CARD --}}
            <div
                class="w-full relative bg-[#0a0a0a]/90 backdrop-blur-md border border-white/10 p-6 sm:p-8 clip-corner-nex shadow-2xl overflow-hidden">
                ...
                <form method="POST" action="{{ route('otp.process') }}" class="space-y-4 relative z-30">
                    ...
                    <div class="space-y-2">
                        ...
                        @error('otp')
                            <p class="text-red-500 text-[9px] font-bold uppercase tracking-wider text-center mt-2">
                                {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="pt-4">
                        <button type="submit"
                            ...
                            <span
                                class="material-symbols-outlined text-[16px] group-hover:scale-110 transition-transform">fingerprint</span>
                        </button>
                    </div>
                </form>
            </div>
            <div class="mt-6 text-center">
                <form action="{{ route('otp.resend') }}" method="POST" id="resend-form">
                    @csrf
                    {{-- PENANDA BAHWA INI RESEND UNTUK REGISTER --}}
                    <input type="hidden" name="type" value="register">
                    
                    <p class="text-[10px] text-slate-500 uppercase tracking-widest mb-2">Tidak menerima kode?</p>
                    <button type="submit" id="resend-btn" disabled
                        class="text-[11px] font-black uppercase tracking-[0.2em] text-cyan-500 hover:text-white disabled:text-slate-700 transition-colors">
            ...
            {{-- FOOTER INFO --}}
            <div
                class="w-full mt-5 flex justify-between items-center px-4 text-[8px] font-mono text-slate-700 uppercase tracking-[0.1em] opacity-50">
